...
class RobotJoystickCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this.config = {
      ...
      timer_count: 6,
      timer1_hour_entity: "", timer1_minute_entity: "", timer1_enabled_entity: "", timer1_action_entity: "",
      timer2_hour_entity: "", timer2_minute_entity: "", timer2_enabled_entity: "", timer2_action_entity: "",
      timer3_hour_entity: "", timer3_minute_entity: "", timer3_enabled_entity: "", timer3_action_entity: "",
      timer4_hour_entity: "", timer4_minute_entity: "", timer4_enabled_entity: "", timer4_action_entity: "",
      timer5_hour_entity: "", timer5_minute_entity: "", timer5_enabled_entity: "", timer5_action_entity: "",
      timer6_hour_entity: "", timer6_minute_entity: "", timer6_enabled_entity: "", timer6_action_entity: "",
      ...config,
    };
  ...
  _renderBatteryIcon(percentage) {
    ...
    return `<div class="battery-icon ${cls}">
  ...
  _getRobotMode() {
    ...
    const tracking      = trackingNorm === "tracciatura filo";
  ...
  _startGrassAnimation() {
    ...
    const animate = (now) => {
      if (!this.statusEls?.grassCut) { this._grassTimer = null; return; }
  ...
  _bindTimerEvents(root) {
    ...
    if (timerCloseBtn) {
      timerCloseBtn.addEventListener("click", () => { this.timerPanelOpen = false; this._updatePanels(); });
    }
    if (timerOverlay) {
      timerOverlay.addEventListener("click", () => { this.timerPanelOpen = false; this._updatePanels(); });
    }
    if (timerSaveBtn) {
      timerSaveBtn.addEventListener("click", () => {
        ...
        if (this.statusEls?.timerSaveMsg) {
          ...
          this._timerSaveMsgTimeout = setTimeout(() => {
            if (this.statusEls?.timerSaveMsg) { this.statusEls.timerSaveMsg.textContent = ""; }
          }, 2500);
        }
  ...
  _refreshTimerUI() {
    ...
    this.timerState.timers.forEach((timer) => {
      ...
      if (summaryEl) {
        summaryEl.textContent = `${String(timer.hour).padStart(2,"0")}:${String(timer.minute).padStart(2,"0")} • ${timer.enabled ? "Attivo" : "Off"} • ${this._actionLabel(timer.action)}`;
      }
    });
  }
  ...
  _renderCard() {
    ...
    root.innerHTML = `
      <style>
        ha-card {
          position: relative; padding: 14px; box-sizing: border-box;
          user-select: none; -webkit-user-select: none; overflow: hidden;
          border-radius: 22px;
          background: radial-gradient(circle at top left, rgba(80,120,255,0.12), transparent 32%),
        ...
        .hero-fade { position: absolute; inset: 0; background: linear-gradient(180deg, rgba(0,0,0,0.16) 0%, rgba(0,0,0,0.04) 30%, rgba(0,0,0,0.20) 100%); pointer-events: none; }
        .info-chip { position: absolute; min-width: 92px; max-width: 120px; padding: 7px 9px; border: 1px solid rgba(255,255,255,0.12); border-radius: 12px; background: rgba(20,20,22,0.52); backdrop-filter: blur(6px); box-shadow: 0 6px 18px rgba(0,0,0,0.18); z-index: 2; }
        .info-label { font-size: 10px; color: #aeb4bf; text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 3px; line-height: 1.1; }
        .info-value { font-size: 13px; font-weight: 800; color: #fff; display: flex; align-items: center; gap: 6px; line-height: 1.2; word-break: break-word; }
        ...
        .battery-line { display: flex; align-items: center; justify-content: space-between; gap: 6px; width: 100%; }
        .battery-icon { display: flex; align-items: center; flex-shrink: 0; }
        .battery-shell { width: 46px; height: 21px; border: 2px solid #a1a1a1; border-radius: 6px; background: #e2e2e2; padding: 2px; box-sizing: border-box; }
        .battery-bars { width: 100%; height: 100%; display: grid; grid-template-columns: repeat(5, 1fr); gap: 2px; }
        .battery-bar { border-radius: 2px; background: #bbbbbb; }
        .battery-bar.active { background: #24c95b; }
        .battery-icon.mid .battery-bar.active { background: #e8b21f; }
        .battery-icon.low .battery-bar.active { background: #ff4d4d; }
        ...
        .mower-mini .body { position: absolute; left: 18px; bottom: 6px; width: 30px; height: 12px; background: #d4a53a; border-radius: 6px 10px 4px 4px; }
        .mower-mini .body-top { position: absolute; left: 22px; bottom: 14px; width: 24px; height: 9px; background: #d4a53a; border-radius: 5px 7px 3px 3px; }
        .mower-mini .front-arm { position: absolute; left: 43px; bottom: 9px; width: 10px; height: 6px; background: #8d8d8d; border-radius: 2px; }
        .mower-mini .front-wheel { position: absolute; right: 0; bottom: 1px; width: 12px; height: 12px; border-radius: 50%; background: #d98c2e; border: 2px solid rgba(0,0,0,0.45); box-sizing: border-box; }
        .mower-mini .sensor-head { position: absolute; left: 30px; bottom: 20px; width: 11px; height: 6px; background: #e7e2d8; border-radius: 2px 2px 0 0; border: 1px solid rgba(0,0,0,0.18); box-sizing: border-box; }
        .mower-mini .trim-cut { position: absolute; left: 18px; bottom: 2px; width: 26px; height: 2px; background: rgba(255,255,255,0.35); border-radius: 2px; }
        @keyframes robotPass { 0% { left: -62px; } 100% { left: calc(100% - 4px); } }
        ...
        .panel-close { width: 40px; height: 40px; border-radius: 12px; border: 0; cursor: pointer; color: #fff; background: rgba(255,255,255,0.08); font-size: 18px; font-weight: 800; }
        .panel-note { font-size: 12px; opacity: 0.76; margin-bottom: 12px; }
        .top-wrap { display: flex; justify-content: center; align-items: center; }
        .pad { position: relative; width: 260px; height: 260px; border-radius: 50%; background: radial-gradient(circle at center, rgba(255,255,255,0.04) 0 24%, transparent 25%), radial-gradient(circle at center, rgba(255,255,255,0.03) 0 62%, rgba(255,255,255,0.07) 63%, transparent 64%), rgba(255,255,255,0.02); overflow: hidden; touch-action: none; border: 1px solid rgba(255,255,255,0.06); box-shadow: inset 0 0 40px rgba(255,255,255,0.03); }
        .cross-h, .cross-v { position: absolute; background: rgba(255,255,255,0.06); }
        .cross-h { left: 14%; right: 14%; top: 50%; height: 1px; transform: translateY(-50%); }
        .cross-v { top: 14%; bottom: 14%; left: 50%; width: 1px; transform: translateX(-50%); }
        .arrow { position: absolute; width: 0; height: 0; opacity: 0.85; }
        .arrow.up    { left: 50%; top: 18px;    transform: translateX(-50%); border-left: 10px solid transparent; border-right: 10px solid transparent; border-bottom: 18px solid #a7a7c7; }
        .arrow.down  { left: 50%; bottom: 18px; transform: translateX(-50%); border-left: 10px solid transparent; border-right: 10px solid transparent; border-top: 18px solid #a7a7c7; }
        .arrow.left  { top: 50%; left: 18px;   transform: translateY(-50%); border-top: 10px solid transparent; border-bottom: 10px solid transparent; border-right: 18px solid #a7a7c7; }
        .arrow.right { top: 50%; right: 18px;  transform: translateY(-50%); border-top: 10px solid transparent; border-bottom: 10px solid transparent; border-left: 18px solid #a7a7c7; }
        .knob { position: absolute; left: 50%; top: 50%; width: 70px; height: 70px; border-radius: 50%; transform: translate(-50%, -50%); background: #a8acc6; box-shadow: 0 0 0 12px rgba(255,255,255,0.03), 0 6px 20px rgba(0,0,0,0.35); }
        .telemetry { text-align: center; margin-top: 12px; line-height: 1.7; font-size: 14px; color: var(--secondary-text-color); }
        .timer-list { display: grid; gap: 10px; }
        .timer-row { border-radius: 18px; padding: 12px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.08); }
        .timer-row-head { display: flex; justify-content: space-between; align-items: flex-start; gap: 10px; margin-bottom: 10px; }
        .timer-label { font-weight: 800; font-size: 15px; }
        .timer-summary { font-size: 12px; opacity: 0.78; text-align: right; }
        .timer-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; }
        .field { display: grid; gap: 6px; }
        .field-wide { grid-column: 1 / -1; }
        .field span { font-size: 12px; opacity: 0.8; }
        .timer-input { width: 100%; border-radius: 12px; border: 1px solid rgba(255,255,255,0.10); background: rgba(255,255,255,0.08); color: #fff; padding: 10px 12px; outline: none; font-size: 14px; font-weight: 700; }
        .timer-input option { color: #111; }
        .timer-footer { margin-top: 14px; display: grid; gap: 8px; }
        .timer-save-msg { min-height: 18px; font-size: 12px; color: #9cffb2; text-align: center; }
        @media (max-width: 700px) {
          .hero { min-height: 340px; } .pad { width: 220px; height: 220px; }
          .button-grid-4 { grid-template-columns: repeat(2, 1fr); }
          .info-chip { min-width: 78px; max-width: 104px; padding: 6px 8px; border-radius: 10px; }
          .info-label { font-size: 9px; } .info-value { font-size: 11px; gap: 4px; }
          .chip-battery { top: 10px; left: 8px; } .chip-loop { left: 8px; bottom: 72px; } .chip-amps { right: 8px; bottom: 72px; min-width: 86px; max-width: 96px; }
          .battery-shell { width: 36px; height: 18px; padding: 2px; } .battery-tip { width: 4px; height: 8px; margin-left: 2px; }
          .mow-panel { left: 10px; right: 10px; bottom: 10px; height: 52px; border-radius: 14px; }
          .mow-text { left: 12px; top: 7px; font-size: 15px; } .mow-sub { left: 12px; bottom: 7px; font-size: 10px; }
          .grass-track { left: 112px; right: 8px; top: 7px; bottom: 7px; }
          .mower-mini { width: 44px; height: 24px; left: -50px; }
          .mower-mini .rear-wheel { width: 19px; height: 19px; } .mower-mini .wheel-guard { width: 22px; bottom: 9px; }
          .mower-mini .body { left: 14px; width: 24px; height: 10px; bottom: 5px; } .mower-mini .body-top { left: 17px; width: 19px; height: 7px; bottom: 12px; }
          .mower-mini .front-arm { left: 34px; width: 8px; height: 5px; bottom: 8px; } .mower-mini .front-wheel { width: 10px; height: 10px; }
          .mower-mini .sensor-head { left: 23px; width: 9px; height: 5px; bottom: 17px; } .mower-mini .trim-cut { left: 14px; width: 20px; }
      ...
      <ha-card>
        ...
        <div class="slide-panel" id="joystick_panel">
          ...
          <div class="panel-note">Muovi il joystick per comandare il robot e premi X per tornare alla card principale.</div>
          <div class="top-wrap">
            <div>
        ...
        <div class="slide-panel" id="timer_panel">
          ...
          <div class="panel-note">Modifica gli orari e le azioni, poi premi conferma per salvarli sul robot.</div>
          <div class="timer-list">${this._renderTimerRows()}</div>
    ...
    const stopDrag = (ev) => {
      if (!this.isDragging) return;
      this.isDragging = false;
      try { this.pad.releasePointerCapture(ev.pointerId); } catch (_e) {}
      this._stopJoystickPublishLoop();
      this._reset();
    };
