...
class RobotJoystickCard extends HTMLElement {
  ...
  _getLastChanged(entityId) {
    const lc = this._hass?.states?.[entityId]?.last_changed;
  ...
  _getRobotMode() {
    ...
    const dockedNow        = dockedNorm === "in base";
    ...
    if (dockedNow)        return { code: "docked",        label: "In base",        sublabel: "In attesa di ricarica",  animate: false, panelOff: true  };
    if (parkedNow)        return { code: "parked",        label: "Parcheggiato",   sublabel: "Robot fermo",            animate: false, panelOff: true  };
